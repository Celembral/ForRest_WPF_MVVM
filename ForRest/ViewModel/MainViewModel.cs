...
namespace ForRest.ViewModel
{
    public class MainViewModel : BindableBase
    {
        ...
        {
            conn = new SqlConnection("Server=(localdb)\\Celembral;Database=ForRest1;Trusted_Connection=True;Integrated Security=true");
            string q_select = String.Format("SELECT state FROM orders WHERE Order_no={0}", Details[0].Order_no);
            try
            {
                conn.Open();
                SqlCommand cmd = new SqlCommand(q_select, conn);
        ...
        private void CheckLogin(object parameter1)
        {
            ...
            if ((Username_VM == null || Username_VM == "") && (password == null || password == ""))
            {
                ...
                string logstrenter = Display_Message;
                logger.Info(logstrenter);
            }
            else if (Authenticate(Username_VM, password) == false)
            {
                Display_Message = "Username or password is incorrect!";
                string logstr = "Username: " + Username_VM + " passwd: " + password + " " + Display_Message + "";
                logger.Info(logstr);
            }
            else
            {
                Display_Message = "Succesfully logged in!";
                string logstrsuccess = "Username: " + Username_VM + " passwd: " + password + " " + Display_Message + "";
                logger.Info(logstrsuccess);
            }
        ...
        private void CheckOut()
        {
            ...
            if (state_global == "In Progress")
            {
                ...
                try
                {
                    ...
                    conn.Close();
                    logger.Info(Display_Message + "  "+ q + "  ");
                }
                catch (Exception ex)
                {
                    MessageBox.Show(ex.Message);
                    logger.Error(ex, "exeption Occured!");
            ...
            else if (state_global == "Canceled")
            {
                ...
                logger.Info(Display_Message + "  " + state_global + "  ");
            }
            else
            {
        ...
        private void Cancel_order()
        {
            ...
            if (state_global == "In Progress")
            {
                ...
                try
                {
                    ...
                    conn.Close();
                    logger.Info(Display_Message + " " + q_update + " ");
                }
                catch (Exception ex)
                {
                    MessageBox.Show(ex.Message);
        ...
        private void Print_Bill()
        {
            ...
            if (state_global == "Completed")
            {
                ...
                logger.Info(Display_Message + "  " + doc + "");
            }
            else if (state_global == "Canceled")
            {
        ...
        private FlowDocument CreateFlowDocument()
        {
            ...
            p = new Paragraph(new Run("Total Price: " + Details[0].Price.ToString() + "Руб."));
            doc.Blocks.Add(p);
        ...
        private void Select_Details()
        {
            ...
            conn = new SqlConnection("Server=(localdb)\\Celembral;Database=ForRest1;Trusted_Connection=True;Integrated Security=true");
            string q_select = String.Format("use ForRest1 SELECT od.ID, od.Order_no, o.Table_no, f.Name as Food, SUM(f.Price * od.Count) as Price, o.Cancel_Reason, w.Realname as Waiter, o.Customer, o.State FROM order_details od INNER JOIN food_list f ON f.Food_ID = od.Food INNER JOIN orders o ON o.Order_no = od.Order_no INNER JOIN workers w ON w.Worker_ID = o.Waiter where o.Order_no = {0} GROUP BY od.Order_no, od.ID, o.Table_no,f.Name,o.Cancel_Reason,w.Realname,o.Customer,o.State", (Row_num_order+1));
            try
            {
                conn.Open();
                SqlCommand cmd = new SqlCommand(q_select, conn);
        ...
        private void Select_More_Details()
        {
            ...
            {
                conn.Open();
                SqlCommand cmd2 = new SqlCommand(q_select, conn);
        ...
        private void Edit_Details_Grid()
        {
            ...
            if (state_global == "In Progress")
            {
                ...
                string q_update = String.Format("UPDATE orders SET Table_no={0}, Waiter={1} WHERE Order_no={2}", (Selected_table + 1), (Selected_waiter + 1), Details[0].Order_no);
                try
                {
                    conn.Open();
                    SqlCommand cmd1 = new SqlCommand(q_delete, conn);
                    cmd1.ExecuteNonQuery();

                    for (int i = 0; i < Details2.Count; i++)
                    {
                        q_insert_orderdetails = String.Format("INSERT INTO order_details(Order_no, Food, Count) VALUES(" + Details[0].Order_no + ", " + Details2[i].Food_ID + ", " + Details2[i].Count + ")");
                        SqlCommand cmd2 = new SqlCommand(q_insert_orderdetails, conn);
                        cmd2.ExecuteNonQuery();
                    }
                    SqlCommand cmd3 = new SqlCommand(q_update, conn);
        ...
        private void Delete_from_details()
        {
            ...
            if (state_global == "In Progress")
            {
                if (Details2.Count != 0)
                {
                    ...
                    {
                        conn.Open();
                        SqlCommand cmd = new SqlCommand(q_delete, conn);
                        cmd.ExecuteNonQuery();
                        conn.Close();
                    }
                    catch (SqlException ex)
        ...
        private void Delete_User()
        {
            if (Workers.Count != 0)
            {
                ...
                try
                {
                    ...
                    dataReader2.Close();
                    string user = name[Row_num].ToString();
                    string q = String.Format("DELETE FROM workers WHERE Username='{0}'", user);
        ...
        {
            conn = new SqlConnection("Server=(localdb)\\Celembral;Database=ForRest1;Trusted_Connection=True;Integrated Security=true");
            string query = String.Format("INSERT INTO workers(Username, Password, Registration_Date, Salary, Title, Realname) VALUES('" + New_Username + "', '" + New_Passwordd + "', GETDATE(), " + Salary + ", '" + Title + "', '" + Real_Name + "')");
            try
            {
                conn.Open();
        ...
        private void New_Order()
        {
            ...
            {
                conn = new SqlConnection("Server=(localdb)\\Celembral;Database=ForRest1;Trusted_Connection=True;Integrated Security=true");
                string q_insert_orders = String.Format("INSERT INTO orders(Table_no, State, Waiter, Time) VALUES(" + (Selected_table + 1) + ", 'In Progress', " + (Selected_waiter + 1) + ", GETDATE())");
                ...
                try
                {
                    ...
                    SqlCommand cmd1 = new SqlCommand(q_select_max, conn);
                    ...
                    SqlCommand cmd2 = new SqlCommand(q_select_state, conn);
                    SqlDataReader dataReader2 = cmd2.ExecuteReader();
                    while (dataReader2.Read())
                    {
                        _table_free = false;
                    ...
                    if (_table_free == true)
                    {
                        SqlCommand cmd3 = new SqlCommand(q_insert_orders, conn);
                        cmd3.ExecuteNonQuery();

                        for (int i = 0; i < New_Order_Details.Count; i++)
                        {
                            q_insert_orderdetails = String.Format("INSERT INTO order_details(Order_no, Food, Count) VALUES(" + next_order_num + ", " + New_Order_Details[i].Food_ID + ", " + New_Order_Details[i].Count + ")");
                            SqlCommand cmd4 = new SqlCommand(q_insert_orderdetails, conn);
        ...
        private void Add_button()
        {
            if (New_Order_Details.Count != 0)
            {
                ...
                {
                    New_Order_Details.Add(new New_order_details
                    {
        ...
        private void NewO_Add_button()
        {
            ...
            if ((n == false && z == true) || empty == true)
            {
                New_Order_Details.Add(new New_order_details
                {
                    ...
                    Food_ID = Selected_food_index + 1,
                    Price = ListofFood[Selected_food_index].Price
                });
            }
        }
        private void OD_Add_button()
        {
            ...
            if (Details[0].State == "In Progress")
            {
                for (int i = 0; i < Details2.Count; i++)
                {
                    ...
                    {
                        Details2.RemoveAt(i);
                        Details2.Add(new New_order_details
                        {
                            Food = Selected_food,
                            Count = Selected_Count + u,
                            Food_ID = Selected_food_index + 1,
        ...
        private void Add_new_food()
        {
            ...
            for (int i = 0; i < ListofFood.Count; i++)
            {
                if (ListofFood[i].Name == Food_name)
                {
                    ...
                    try
                    {
                        ...
                        conn.Close();
                        logger.Info(Display_Message, " Update food (AddNewFood)");
                    }
                    catch (Exception ex)
                    {
                        MessageBox.Show(ex.Message);
            ...
            if (counter == 0)
            {
                string query = String.Format("INSERT INTO food_list(Name, Price, Category, In_Stock) VALUES('" + Food_name + "', " + Food_price + ", '" + Category + "', " + In_Stock + ")");
                try
                {
                    conn.Open();
                    SqlCommand cmd = new SqlCommand(query, conn);
        ...
        private void View_Waiters()
        {
            ...
            else
            {
                ...
                logger.Info(Display_Message + "  " + _is_manager + " View waiters");
            }
        ...
        private void View_New_Food()
        {
            ...
            {
                Display_Message = "Manager only!";
                logger.Info(Display_Message + "  " + _is_manager + " ViewNewFood");
            }
        }
